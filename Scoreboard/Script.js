...
const STAT_MAX = 5;              // fouls and timeouts each cap out at this number
...
const state = {
  ...
  teams: {
    A: { name: "", photo: "", score: 0, foul: 0, to: 3 },
    B: { name: "", photo: "", score: 0, foul: 0, to: 3 }
  }
};
...
function renderPeriod() {
  ...
  periodLabel.textContent = idx < 4 ? PERIOD_NAMES[idx] : `OT${idx - 3}`;
}
...
function renderTeam(team) {
  ...
  // Foul/timeout counters cap at STAT_MAX: grey out the "+" button and mark
  // the number so it's obvious a limit's been hit.
  updateStatCap(team, "foul", t.foul);
  updateStatCap(team, "to", t.to);
}

function updateStatCap(team, stat, value) {
  const valueEl = el(`${stat}${team}`);
  const plusBtn = document.querySelector(`.stat-btn.plus[data-team="${team}"][data-stat="${stat}"]`);
  const atLimit = value >= STAT_MAX;
  valueEl.classList.toggle("at-limit", atLimit);
...
function adjustStat(team, stat, dir) {
  const t = state.teams[team];
  if (stat === "foul") {
    t.foul = Math.min(STAT_MAX, Math.max(0, t.foul + dir));
  } else if (stat === "to") {
    t.to = Math.min(STAT_MAX, Math.max(0, t.to + dir));
  }
  renderTeam(team);
  queueSync("stat", team);
}
...
function resetMatch() {
  ...
  ["A", "B"].forEach((team) => {
    ...
    state.teams[team].foul = 0;
    state.teams[team].to = 3;
  });
  renderAll();
  queueSync("full");
}
  ...
  try {
    ...
    Object.entries(map).forEach(([id, team]) => {
      ...
      if (buf) {
        ...
        state.teams[team].to = Number(buf['T/O']) ?? 3;
      }
      if (trow) {
